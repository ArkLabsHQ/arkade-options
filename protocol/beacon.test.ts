...
import { arkade, asset, Extension, SingleKey, Transaction } from "@arkade-os/sdk";
...
import { buildFixture, FIXTURE, fixtureJson } from "./beacon-fixture.ts";
import { encodeWitness, statePacket } from "./cospend.ts";
...
const key = (byte: number) => SingleKey.fromHex(byte.toString(16).padStart(2, "0").repeat(32));
...
test("the committed VM fixture is what the builder produces", async () => {
  const committed = readFileSync(here("../contracts/vm/testdata/settle.json"), "utf8");
  assert.equal(await fixtureJson(), committed);
});
